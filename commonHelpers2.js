import{d as x,b as $,h as C,s as W,a as v,c as U,o as N,n as H}from"./assets/change-theme-90b3de1b.js";import{i as y}from"./assets/vendor-0a7943b3.js";const k=async e=>{const t=i=>{const n=i;if(n==="Muscles")return n.toLowerCase();if(n==="Body parts")return"bodypart";if(n==="Equipment")return n.toLowerCase()},s={...e},a=await x.getFilters(s),r=a.data.results;return $(a.data.totalPages,e.page),r.reduce((i,n)=>i+` <li class="render-page-one-item" data-filter="${t(e.filter)}" data-name="${n.name}">
...
</p>`;le.innerHTML=e,ce.style.display="none"},ue=getComputedStyle(document.querySelector("body")).width,pe=parseInt(ue)<768?8:9,me=document.querySelector(".form-search"),L=document.querySelector(".render-page-one-list"),m=document.querySelector(".render-btn-list-pagination"),ge=(e,t)=>{let s="";for(let a=1;a<=e;a++){let r=` <li>
        <button
          class="render-pagination-btn-search"
          type="button"
        >${a}</button></li>`;s+=r}m.innerHTML=s,m.children[t-1].children[0].classList.add("render-pagination-btn-active")},c={limit:pe,page:1},fe=async e=>await v.get("exercises",{params:e}),O=async()=>{m.style.display="flex";const e=await fe(c);if(!e.data.results.length){de();return}g(F(e.data.results,c)),ge(e.data.totalPages,c.page)},ye=async e=>{e.preventDefault();const t=L.children[0].dataset.filter,s=L.children[0].dataset.name;c[t]=s,c.keyword=e.target.query.value,m.addEventListener("click",ve),O()},ve=e=>{c.page=parseInt(e.target.textContent),O()};me.addEventListener("submit",ye);g(k({page:1,filter:"Muscles"}));const ke=document.querySelector(".form");ke.addEventListener("submit",ae);H();
